...
const byBirth = inventors.slice().sort((a, b) => a.year - b.year);
console.table(byBirth);

// Array.prototype.reduce()
// 4. How many years did all the inventors live?
const lifeSpan = inventors.reduce((sum, inventor) => sum + (inventor.passed - inventor.year), 0);

// 5. Sort the inventors by years lived
const byLifeSpan = inventors.slice().sort((a, b) => (a.passed - a.year) - (b.passed - b.year));

// 6. create a list of Boulevards in Paris that contain 'de' anywhere in the name
// https://en.wikipedia.org/wiki/Category:Boulevards_in_Paris
const links = Array.from(document.querySelectorAll('.mw-category-group a'));
const boulevards = links.map(l => l.textContent).filter(name => name.includes('de'));

// 7. sort Exercise
// Sort the people alphabetically by last name
const byName = people.sort((a, b) => a.split(', ')[0] < b.split(', ')[0] ? -1 : 1);
console.log(byName);

// 8. Reduce Exercise
// Sum up the instances of each of the objects
const occurance = objects.reduce((acc, item) => {
  if (!acc[item]) {
    acc[item] = 0;
  }
